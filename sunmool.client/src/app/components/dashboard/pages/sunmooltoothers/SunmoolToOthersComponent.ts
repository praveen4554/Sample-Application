...
@Component({
  ...
  styleUrls: ['./src/app/components/dashboard/pages/sunmooltoothers/SunmoolToOthers.css'],
  templateUrl: './src/app/components/dashboard/pages/sunmooltoothers/SunmoolToOthers.html',
})
export class SunmoolToOthersComponent {
    ...
    this.af.database.list('/sunmool', {
     ...
     .subscribe(myItems => {
                ...
                for(var i in this.itemsRaw) {
                  if(!auxObj[this.itemsRaw[i].item])
                    auxObj[this.itemsRaw[i].item] = [];

                  auxObj[this.itemsRaw[i].item].push(this.itemsRaw[i]);
                }
